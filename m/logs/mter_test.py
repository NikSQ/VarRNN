...
bn = [True]
samplingop = ['g_sampling'] * 4 + ['l_sampling']* 4

training_config = {'learning_rate': lr[task_id], 
                   'type': samplingop[task_id],
                   'is_pretrain': False,
                   'batchnorm': False,
                   'var_reg': 0.,
                   'ent_reg': 0.,
...
training_config['mode'] = {'name': 'classic', 'min_error': 0., 'max_epochs': 10000} 
...
load_file=['pretrain_mnist_batchnorm_1', 'pretrain_mnist_1']
...
info_config = {'calc_performance_every': 1,
               'n_samples': 100,
               'tensorboard': {'enabled': True, 'path': '../m/tb/' + filename + '_' + str(task_id), 'period': 200,
