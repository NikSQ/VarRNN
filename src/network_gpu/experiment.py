...
class Experiment:
    ...
    def save_gradient_stats(self, sess, gumbel_tau):
        ...
        for gradient_idx in range(n_gradients):
            sess.run(self.rnn.c_arm_sample_op)
            gradient, d1, d2, d3 = sess.run([rnn_gradients, debug1, debug2, debug_sb], feed_dict={self.gpu_dataset.batch_idx: 0,
                                                                self.rnn.is_training: True,
                                                                self.rnn.tau: gumbel_tau})
            samples += np.squeeze(d1)
        ...
        samples = samples / n_gradients
        print((samples + 1) * 0.5)
        print(d3)
        print(1 / (1 + np.exp(-np.squeeze(d3))))
