...
save_results=True
datam = None
#sampling_op = [['c_reparam', 'cste'], ['c_reparam', 'gste'], ['c_reparam'], ['l_reparam'], ['l_reparam'], ['l_reparam', 'ste']][task_id]
#sampling_op = [['c_reparam', 'gste'], ['l_reparam', 'ste']][task_id]

#sampling_op = [['c_reparam', 'gste']][int(task_id / 9)]
#sampling_op = [['c_ar'], ['c_arm'], ['log_der']][int(task_id / 3)]
sampling_op = ['c_arm']
weight_type='binary'
forward_passes = 10

#batchnorm = ['fc', 'x+fc', 'h+fc', 'x+h+fc', 'none'][task_id]
#discrete_act = [[], [], [], [], ['c', 'o'], ['c', 'o']]
#discrete_act = discrete_act[task_id]
#tau = np.logspace(-2, 5, 8)[task_id % 9]
tau = 1.
discrete_act = []
batchnorm = []
batchnorm_type = 'none'
...
w_config = {'parametrization': 'sigmoid', 'priors': priors[1], 'type': weight_type, 'pmin': .01, 'pmax':.99, 'p0min': .05, 'p0max': .95}
w_config = {'parametrization': 'sigmoid', 'priors': priors[1], 'type': weight_type, 'pmin': .01, 'pmax':.99, 'p0min': .05, 'p0max': .95}
...
training_config = {'learning_rate': lr, 
                   ...
                   'pretraining': {'path': 'm_model', 'enabled': True},
